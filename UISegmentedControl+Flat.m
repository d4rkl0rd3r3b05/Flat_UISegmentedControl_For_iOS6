...
- (UIImage*)imageWithColor:(UIColor*)color cornerRadius:(CGFloat)cornerRadius
{
    ...
    return [image resizableImageWithCapInsets:UIEdgeInsetsMake(cornerRadius,
                                                               cornerRadius,
                                                               cornerRadius,
                                                               cornerRadius)];
}
                    ...
                    cornerRadius:(CGFloat)cornerRadius
{
    ...
    UIImage* topImage = [self imageWithColor:color
                                cornerRadius:cornerRadius];
    UIImage* bottomImage =
        [self imageWithColor:shadowColor
                cornerRadius:cornerRadius];
    ...
    UIGraphicsBeginImageContextWithOptions(
        CGSizeMake(totalWidth, totalHeight), NO, 0.0f);
...
- (void)setupFlat:(BOOL)isSegmentToBeGray
{
    ...
    self.frame = CGRectMake(
        self.frame.origin.x, self.frame.origin.y, self.frame.size.width, 29);

    UIColor* normalColor = [UIColor whiteColor];
    UIColor* selectedColor;
    if (isSegmentToBeGray) {
        selectedColor = GRAY_COLOR;
    } else {
    ...
    UIImage* selectedBackgroundImage =
        [self buttonImageWithColor:selectedColor
                      cornerRadius:cornerRadius];
    UIImage* deselectedBackgroundImage =
        [self buttonImageWithColor:normalColor
                      cornerRadius:cornerRadius];
    ...
    [self setBackgroundImage:deselectedBackgroundImage
                    forState:UIControlStateNormal
                  barMetrics:UIBarMetricsDefault];
    [self setBackgroundImage:disabledBackgroundImage
                    forState:UIControlStateDisabled
                  barMetrics:UIBarMetricsDefault];

    UIImage* dividerImage =
        [Utility imageWithColor:selectedColor
    ...
    [self setTitleTextAttributes:selectedAttributesDictionary
                        forState:UIControlStateSelected];
    ...
    [self setTitleTextAttributes:normalAttributesDictionary
                        forState:UIControlStateNormal];
}
